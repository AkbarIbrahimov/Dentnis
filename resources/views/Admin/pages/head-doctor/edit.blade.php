...
@section('content')
    <div class="container">
        <h2>Edit Team</h2>
        <form action="{{route('admin.HeadDoctorEdit',['id'=>$headDoctor->id])}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="btn">
                                ...
                                <div class="tab-pane fade {{$loop->first ? 'active show' : ''}}" id="tab-{{$lang}}" role="tabpanel"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea type="text" placeholder="Description" name="{{$lang}}[description]"
                                                  class="form-control summernote" id="summernote">{{ old($lang.'.description', $headDoctor->translations->where('language_id', $langId)->first()->description ?? '') }}</textarea>
            ...
            <button type="submit" onclick="this.disabled=true;this.form.submit();">Edit HeadDoctor</button>
        </form>
    </div>
